...
- withdraws the requested complete batches of item inputs for non-crafting recipes, such as a Laser Engraver recipe, into the player inventory;
...
If stock limits the batch count, a chat message reports the available batches and missing item quantities for the full request. It describes one concrete combination of recipe alternatives, not necessarily the smallest possible shopping list. This stock estimate does not confirm delivery; the server still checks power, access, and inventory space. A missing physical catalyst blocks the transfer and is included in the shortage message. Overlapping alternatives use a bounded retry when the initial selection fails; exceptionally complex combinations can still be conservatively rejected to keep interaction responsive.
